...
namespace negocio
{
      ...
      public class ListaProductosNegocio
      {
            public List<ListaProductos> ListarProductosCompra(int id)
            {
                ...
                try
                {
                    datos.setearConsulta("select ListaProductos.IdProducto, ListaProductos.Cantidad, ListaProductos.IdNumeroDeCompra, Producto.Codigo, Producto.NombreProducto from ListaProductos inner join Producto on ListaProductos.IdProducto = Producto.IdProducto and ListaProductos.IdNumeroDeCompra = " + id);
            ...
            public List<ListaProductos> ListarProductosVenta(int id)
            {
                ...
                try
                {
                    datos.setearConsulta("select ListaProductos.IdProducto, ListaProductos.Cantidad, ListaProductos.IdNumeroDeVenta, Producto.Codigo, Producto.Stock, Producto.NombreProducto from ListaProductos inner join Producto on ListaProductos.IdProducto = Producto.IdProducto and ListaProductos.IdNumeroDeVenta = " + id);
                    datos.ejecutarLectura();
                    while (datos.Lector.Read())
                    {
                        ListaProductos aux = new ListaProductos();
            ...
            public void AgregarListaCompra(List<ListaProductos> listaproductos, int ultimonumerocompra)
            {
                ...
                try
                {
                    foreach (ListaProductos item in listaproductos)
                    {
                        ...
                        string valores = "values('" + item.ItemArt.IdProducto + "', '" + item.Cantidad + "', '" + aux.IdNumeroDeCompra + "')";
                        datos.setearConsulta("insert into ListaProductos (IdProducto, Cantidad, IdNumeroDeCompra) " + valores);
        ...
        public void AgregarListaVenta(List<ListaProductos> listaproductos, int ultimonumeroventa)
        {
            ...
            try
            {
                foreach (ListaProductos item in listaproductos)
                {
                    ...
                    string valores = "values('" + item.ItemArt.IdProducto + "', '" + item.Cantidad + "', '" + aux.IdNumeroDeVenta + "')";
                    datos.setearConsulta("insert into ListaProductos (IdProducto, Cantidad, IdNumeroDeVenta) " + valores);
                    datos.ejectutarAccion();
                    datos.cerrarConexion();
                }
